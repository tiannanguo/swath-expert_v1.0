...
lib_part_number_dir = sys.argv[1]   #libPart_1
...
#select peptides
selected_transition = {}
lib_part_csv_file_name = lib_part_number_dir + ".csv"
num_select = 0
...
#get chrom.gz files
glob_string = "./nci60_postgre/" + lib_part_number_dir +"/*.gz"
print "glob ", glob_string
chrom_gz_file_list = glob.glob(glob_string)
#['./nci60_postgre/libPart_1\\guot_L130610_003_SW_libPart_1.mzML.chrom.gz',
in_file_pattern = re.compile (r'./nci60_postgre/(libPart_\d+)\\(.*)_libPart_\d+.mzML.chrom.gz')
m = in_file_pattern.search(chrom_gz_file_list[0])
lib_part_number = m.group(1)
out_file_name = lib_part_number + "_test.allChrom.gz"
