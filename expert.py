...
def swath_peak_group_filter_expert(in_file, transition_groups, this_tg, sample_id,
    ...
    if num_filtered_peak_group == 1 and peak_groups_filtered.keys()[0] > 0:  # one good peak group is found
...
def swath_peak_group_filter_expert_worker(this_tg, rt_list, int_list, peak_group,
    ...
    num_good_transitions, peak_group2 = count_number_of_good_transition_expert(peak_group, reference_sample_num_transitions)
    ...
    if num_good_transitions == 0:
        ...
        if number_highly_correlated_peak_groups == 0:
            ...
            #if there is no ms1 or multiple ms1, then examine peak shape, select the one with highest score
            elif num_good_ms1 == 0 or num_good_ms1 > 1:

                #RULE 4: if multiple peaks found, eg. 76_AAALEAMK_2 , sample MCF7_a, select the one with better peak shape
                peak_group5 = find_best_peak_shape_expert(this_tg, rt_list, int_list, peak_group4, reference_sample_peak_group_info)
...
def compute_fragment_ranking_expert(this_tg, rt_list, int_list, peak_group, reference_sample_peak_group_info):
    ...
    #compute the correlation of the transition intensity of the peak group between this sample and the reference sample
...
def find_best_peak_shape_expert(this_tg, rt_list, int_list, peak_group, reference_sample_peak_group_info):
    ...
    for rt in peak_group:
        ...
        left_intensity, right_intensity = compute_transition_intensity_boundary(rt, rt_list, int_list, reference_sample_peak_width)
...
def examine_ms2_peak_group_shape_expert(rt_list, int_list):
    ...
    ms2_if_good_peak_group_top1, ms2_if_interference_top1 = examine_peak_shape_expert(rt_list[top1_transition], int_list[top1_transition])
...
def examine_cut_out_peak_group_expert(rt_list, int_list):
    ...
    if_ms2_peak_group = 0
    if sum(correlation_of_transition_intensity) / len(correlation_of_transition_intensity) > 0.9:    # RULE x, very stringent
...
def quant_peak_group_expert(display_data):
    ...
    for sample_id in display_data.keys():
        ...
        display_data[sample_id]['if_ms1_good_peak'], display_data[sample_id]['if_ms1_interference_peak'] = \
            examine_peak_shape_expert(display_data[sample_id]['ms1_rt'], display_data[sample_id]['ms1_int'])

        display_data[sample_id]['ms1_area'] = chrom_peak_boundary.compute_peak_area(display_data[sample_id]['ms1_rt'],
                                                                                    display_data[sample_id]['ms1_int'],
                                                                                    display_data[sample_id]['ms1_rt'][0],
                                                                                    display_data[sample_id]['ms1_rt'][-1])

        #ms2 data

        display_data[sample_id]['if_ms2_peak_group'], display_data[sample_id]['if_ms2_interference'] = \
            examine_ms2_peak_group_shape_expert(display_data[sample_id]['ms2_rt'], display_data[sample_id]['ms2_int'])

        #RULE x: if ms1 is good peak, sometimes ms2 are not good. In this case, force the ms2 as good peak
        if display_data[sample_id]['if_ms1_good_peak'] == 1:
            display_data[sample_id]['if_ms2_peak_group'] = 1

        #RULE x: if ms2 does not have good peak group, sometimes because the peak is cut out due to chrom limit. rescue them in this case
        if_peak_group_cut_out = 0
        if display_data[sample_id]['if_ms2_peak_group'] == 0:
            if_peak_group_cut_out, display_data[sample_id]['if_ms2_peak_group'] = \
                examine_cut_out_peak_group_expert(display_data[sample_id]['ms2_rt'], display_data[sample_id]['ms2_int'])
            ...
            if display_data[sample_id]['if_ms2_interference'][this_transition] == 0:
                ...
                display_data[sample_id]['ms2_area'][this_transition] = \
                    chrom_peak_boundary.compute_peak_area(display_data[sample_id]['ms2_rt'][this_transition],
                                                          display_data[sample_id]['ms2_int'][this_transition],
                                                          display_data[sample_id]['ms2_rt'][this_transition][0],
...
def compute_peak_shape_score(left_intensity, peak_intensity, right_intensity):
    ...
    for this_transition in left_intensity.keys():
        score = math.log((peak_intensity[this_transition]+1)/(left_intensity[this_transition]+1), 10)
        score += math.log((peak_intensity[this_transition]+1)/(right_intensity[this_transition]+1), 10)
...
def write_out_quant_data_expert(quant_data_write_out,
    ...
    for sample_id in display_data.keys():
        ...
        quant_data_write_out.append((this_tg, transition_list_csv_string, sample_id,
                                     ...
                                     peak_boundary_display['left'][sample_id], peak_boundary_display['right'][sample_id],
                                     display_data[sample_id]['if_ms2_peak_group'], transition_ms2_area_csv_string,
