...
#This function converts it into a list of float %f.2, csv string for r code
def rt_three_values_to_full_list_float (rt_list):
    ...
    #soemtimes the last data point is lost due to small difference <1 second
...
#but for some peaks at the border of which one boundary is cut out, this function can not detect the peak.
#in this case, this function try to repair the peak.
def extend_rt_border(rt_list, int_list):
    ...
    rt_extended_left = np.arange(rt_list[0] - 5* mean_gap, rt_list[0] - mean_gap+1, mean_gap)  #1 means extend it a bit because sometimes one data point is missing
...
#the results from peakdetect is a list of tuple, this function is to take out the rt information of peaks I need
...
#convert the intensity values into normalized in -100 to 100, and then save in a csv string for r code
def format_and_normalize_intensity (int_list, max_int, MS1_MS2):
    ...
    # int_list5 = map(str, int_list4)
    # return ",".join(int_list5)
