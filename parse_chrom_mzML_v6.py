...
RT_TOLERANCE = 240.0 # 180.0  #3min, -3min ~ 3min

# in_file1 = "2R_water.txt"
# in_file2 = "gold10_ms1.chrom.mzML.gz"
# in_file3 = "AQUA374_ms1.csv"
# out_file = "gold10_ms1.chrom.txt.gz"
...
def decode_int_list(int_list):
    decoded = base64.decodestring(int_list)
    ...
    for tmp in struct.unpack(unpack_format1, decoded):
        ...
        int_list2.append( int (tmp_f ))

    return int_list2

def select_rt_range(rt_list, int_list, best_rt, rt_range):
    rt_list3 = [rt for rt in rt_list if best_rt - rt_range < rt < best_rt + rt_range]
    int_list3 = [int for rt, int in zip (rt_list, int_list) if (rt < best_rt + rt_range and rt > best_rt - rt_range)]

    return rt_list3, int_list3
...
def main():
    ...
    with gzip.open (out_file, 'wb') as OUT_FILE:
        ...
        writer.writerow(('transition_name', 'rt_list', 'int_list',
                        'transition_group_id',
                        'best_rt', 'best_sample', 'best_score',
                        'protein', 'irt', 'precursor_mz', 'transition_mz', 'transition_int'))
                        ...
                        for binary_array_data in binary_array_data_list.iter():
                            ...
                            for binary in binary_array_data.iter (binary_tag):
                                if if_rt_list == 1:
                                    ...
                                    int_list = binary.text
                                    if_int_list = 0

                                    #decode rt and int
                                    rt_list2 = decode_rt_list(rt_list)
                                    int_list2 = decode_int_list (int_list)
                                    ...
                                    #select rt range
                                    rt_list3, int_list3 = select_rt_range (rt_list2, int_list2, best_rt[library_data[transition_name]['transition_group_id']], RT_TOLERANCE)
                                    ...
                                    writer.writerow((transition_name, ','.join(str(p) for p in rt_list4), ','.join(str(p) for p in int_list3),
