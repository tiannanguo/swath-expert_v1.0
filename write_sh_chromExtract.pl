...
open(OUT0,">cp_sh_chromExtraction.sh");

open(OUT1,">run_sh_chromExtraction.sh");

foreach my $i(1..120){

        my $sample="nci$i";

        print OUT0 "cp ~/soNas/nci60/chrom_sh_$sample.sh $sample\/\n";

        print OUT1 "cd $sample; sh chrom_sh_$sample.sh ; cd ..\n";

        open(OUT, ">chrom_sh_$sample.sh");
        my $i=0;
        my $jobId=$sample."_$i";
        print OUT "bsub -R \"rusage[mem=6144]\" -J $jobId \"OpenSwathChromatogramExtractor -rt_norm ../trafo/$sample.rtnorm.trafoXML -in $sample\_ms1.mzML.gz -tr ";
        print OUT "../cell63dda_step6ms1.TraML -out $sample\_ms1.chrom.mzML -rt_extraction_window 300\"\n";

        foreach my $j (1..32){
                my $jobId=$sample."_$j";
                my $k=$j-1;
                my $lastJobId=$sample."_$k";
                print OUT "bsub -R \"rusage[mem=6144]\" -J $jobId -w \"ended\($lastJobId\)\" \"OpenSwathChromatogramExtractor -rt_norm ../trafo/$sample.rtnorm.trafoXML -in $sample\_$j.mzML.gz -tr ";
                print OUT "../cell63dda_step5.TraML -out $sample\_$j.chrom.mzML -rt_extraction_window 300 -is_swath\"\n";
        }
